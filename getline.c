...
ssize_t input_buf(info_t *info, char **buf, size_t *len)
{
	...
	if (!*len) /*when nothing is left in the buffer, occupy it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			if ((*buf)[r - 1] == '\n')
			{
				(*buf)[r - 1] = '\0'; /* remove all trailing current line */
				r--;
			}
...
/**
 * get_input - gets a line, remove from the newline,
 * @info: Parameter structure,
 *
 * Return: bytes read.
 */
ssize_t get_input(info_t *info)
{
	...
	if (r == -1) /* EOF */
		return (-1);
	if (len)	/* there are cmd left in the chain buffer */
	{
	...
	*buf_p = buf; /* else not a chain, pass back buffer from _getline() */
	return (r); /* Return the length of a buffer from _getline() */
}
